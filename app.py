...
if uploaded_image is not None:
    ...
    if st.button('Predict Disease'):
        with st.spinner('Processing the image...'):
            predict_disease(uploaded_image, plant_type)import streamlit as st
import tensorflow as tf
import numpy as np
import time
import warnings
...
def predict_disease(uploaded_image, plant_type):
    ...
    # LOAD: Keras 3 ka safest loading path
    try:
        # Sabse pehle legacy loader se try karenge
        from keras.src.saving import legacy_h5_format
        model = legacy_h5_format.load_model_from_hdf5(model_path)
    except:
        # Fallback agar legacy loader fail ho jaye
        model = keras.models.load_model(model_path, compile=False)
